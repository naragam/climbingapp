...
class Climb: NSObject, NSCoding {
    ...
    static let ArchiveUrl = DocumentsDirectory.appendingPathComponent("climbs")
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        guard let name = aDecoder.decodeObject(forKey: PropertyKey.name) as? String else {
            os_log("Unable to decode the name for a climb object.", log: OSLog.default, type:.debug)
            return nil
        }
        ...
        guard let datetime = aDecoder.decodeObject(forKey: PropertyKey.datetime) as? String else {
            os_log("Unable to decode the datetime for a climb object.", log: OSLog.default, type:.debug)
            return nil
        }
